    ...
    : IConsumer<EntityUpdatedEvent<Product>>, IConsumer<EntityUpdatedEvent<Category>>, IConsumer<EntityUpdatedEvent<Manufacturer>>
{
    ...
    {
        await cacheManager.RemoveByPrefixAsync(string.Format(PageCacheConstants.PDP_CACHE_KEY_PREFIX, eventMessage.Entity.Id));
    }

    public async Task HandleEventAsync(EntityUpdatedEvent<Category> eventMessage)
    {
        await cacheManager.RemoveByPrefixAsync(string.Format(PageCacheConstants.CATEGORY_PAGE_CACHE_KEY_PREFIX, eventMessage.Entity.Id));
    }

    public async Task HandleEventAsync(EntityUpdatedEvent<Manufacturer> eventMessage)
    {
        await cacheManager.RemoveByPrefixAsync(string.Format(PageCacheConstants.MANUFACTURER_PAGE_CACHE_KEY_PREFIX, eventMessage.Entity.Id));
    }
}
